    ...
    public abstract class ApproveMasterRepository : MasterRepository, IApproveEntry
    {
        ...
        private bool PassCondition(string tableName, string pkFiled, string refId, string pathConditon)
        {
                ...
                pathConditon.StartsWith("="))
            {
                string sql = "";
                pathConditon=DataConvert.ToString(pathConditon);
                string[] conditions =Regex.Split(pathConditon,"and",RegexOptions.IgnoreCase);
                string valSql = " and (";
                foreach (var c in conditions)
                {
                    valSql += " assetsValue" + c + " and";
                }
                valSql = valSql.Substring(0, valSql.Length - 3) + ")";
                ...
                {
                    sql = string.Format(@" select count(1) cnt from 
                       (select isnull(max(a.assetsValue),0) assetsValue from {0} s,{0}Detail d,Assets a
                        where s.{1} =d.{1} 
                        and d.assetsId=a.assetsId
                        and s.{1}='{2}') M 
                        where 1=1 
                        {3}", DataConvert.ToString(tableName), DataConvert.ToString(pkFiled),
                                                    DataConvert.ToString(refId), valSql);
                }
