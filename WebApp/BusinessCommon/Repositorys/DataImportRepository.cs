    ...
    public class DataImportRepository : BaseRepository
    {
        ...
        private int ImportStoreSite(DataTable dtSource, UserInfo sysUser, string viewTitle)
        {
            ...
            foreach (DataRow drSource in dtSource.Rows)
            {
                ...
                string id = IdGenerator.GetMaxId(dt.TableName);
                dr["storeSiteId"] = id;
                string storeSiteNo = DataConvert.ToString(drSource[0]);
                dr["storeSiteNo"] = storeSiteNo;
                if (storeSiteNo == string.Empty)
                {
                    break;
                }
        ...
        private int ImportAssets(DataSet dsSource, UserInfo sysUser, string viewTitle)
        {
            ...
            foreach (DataTable dtSource in dsSource.Tables)
            {
                ...
                foreach (DataRow drSource in dtSource.Rows)
                {
                    ...
                    try
                    {
                        var pDate = Convert.ToDateTime(drSource["入库日期"]);
                        dr["purchaseDate"] = pDate;
                    }
                    catch (Exception) //如果转时间格式出错，增加些辅助转换功能。
                    {
                        ...
                        purchaseDate = purchaseDate.Replace("-", "/");
                        string[] pds = purchaseDate.Split('/');
                        if (pds.Length == 1)
                            purchaseDate = purchaseDate + "/1/1";
                        ...
                        if (purchaseDate != "")
                        {
                            try
                            {
                                var isdate = Convert.ToDateTime(purchaseDate);
                                dr["purchaseDate"] = purchaseDate;
                            }
                            catch (Exception)
                            {
                                dr["purchaseDate"] =DateTime.Today;
                            }
                        }
                        else
                            dr["purchaseDate"] = DateTime.Today;
                    }
                    dr["addDate"] = dr["purchaseDate"];
                    #endregion
                    ...
                    #region 资产数量，写入datatable
                    if (importAssetsQtyNeedSplitConfig.ToLower() == "true")//导入的资产需要根据资产数量拆分
                    {
                        dr["assetsQty"] = 1;
                        if (isUpdateAssets)//资产编号相同的做更新操作。
                        {
                        ...
                        {
                            int assetQty = DataConvert.ToInt32(drSource["数量"]);
